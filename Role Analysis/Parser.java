...
public class Parser extends Object{
   ...
   public Parser(Chario c, Scanner s, boolean sc, boolean ro){
      chario = c;
      scanner = s;
      this.scope = sc;
      this.role = ro;
   ...
   */
   ...
   private void acceptRole(SymbolEntry s, int expected, String errorMessage){
      if (s.role != SymbolEntry.NONE && s.role != expected && this.role)
         chario.putError(errorMessage);
   }

   private void acceptRole(SymbolEntry s, Set<Integer> expected, String errorMessage){
      if (s.role != SymbolEntry.NONE && ! (expected.contains(s.role)) && this.role)
         chario.putError(errorMessage);
   }
   ...
   */
   ...
   private void initTable(){
      table = new SymbolTable(chario, scope, role);
      table.enterScope();
